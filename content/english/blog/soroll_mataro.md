        ...
        <section id="scrolly-2-5">
            <div class="sticky-thing2-5">
                <div id="titol-piramide">
                    <p style="color:#595959;font-family:Fjalla One;">NUMBER OF PEOPLE AFFECTED BY NOISE POLLUTION BY GENDER AND AGE</p>
                    <p style="margin-top:0;">💡<em>Interact with the chart</em></p>
                </div>
                <div id="map2-5">
                </div>
                <div id="tooltip"></div>
                ...
                .on("mouseover", (event, d) => {
                    ...
                    if (baseColor) {
                        tooltip
                            .style("display", "block")
                            .style("background-color", `rgba(${baseColor.r}, ${baseColor.g}, ${baseColor.b}, 0.8)`)
                            .html(`${d.sexo}<br> Noise levels: ${d.key}<br> Affected: ${Math.abs(d[1] - d[0])}`);
                    }
                })
                .on("mousemove", (event) => {
                    tooltip.style("left", `${event.clientX + 20}px`)
